 ...
 * Reporting Service
 *
 * API client for reporting domain.
...
const rawReportingService = {
  /**
   * Get compliance export bundle
   */
  async getCompliance(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/reports/compliance` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getReport(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/reports` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async createReport(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/reports`;
   ...
   * Get period report
   */
  async getReport(reportId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/reports/${reportId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
...
export const reportingService = makeService(rawReportingService, "reporting");
