...
const rawAuditsService = {
  /**
   * List audit events
   */
  async getEvent(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/audits/events` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getEvent(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/audits/events`;
   ...
   * Get audit event
   */
  async getEvent(eventId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/audits/events/${eventId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getVerifyChain(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/audits/verify-chain`;
...
export const auditsService = makeService(rawAuditsService, "audits");
