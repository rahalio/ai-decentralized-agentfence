...
OpenAPI-first DDD monorepo for **Agentfence** — identity, audit, and autonomy-boundary control plane for autonomous AI agents.

Package scope: **`@agentfence/*`**. Product specs: [PRODUCT.md](PRODUCT.md), [USER_STORIES.md](USER_STORIES.md), [WEBAPP.md](WEBAPP.md).
...
# Demo key: X-API-Key: agentfence_demo_local_dev_key
```
...
```bash
docker compose up -d
TABLE_NAME=agentfence-core-local AWS_ENDPOINT_URL=http://localhost:8000 node scripts/ensure-dynamo-table.mjs
```
